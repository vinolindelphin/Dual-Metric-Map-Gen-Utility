...
METRICS = {
    ...
    "CMS Opportunity Size" : "CMS_MARKET_SIZE",
    "RW to CO Ratio" : "RW_CO_RATIO",
    "SP Usage Churn %" : "SP_USAGE_CHURN_PERC"
}
...
EPS = 1e-6  # define once near the top
...
METRIC_BINS = {
    ...
    "low":  (-np.inf, 10),
    "med":  (10, 40),
    "high": (40, np.inf),   # note: 40.0 becomes High with current binning
    },
...
# KPIs where "lower is better" for achievement (so bins must be flipped for coloring)
REVERSE_ACHIEVEMENT_METRICS = {"SP Usage Churn %"}

# Flip mapping: low-value bin should behave like "high" (good), and vice-versa
ACH_BIN_FLIP = {"low": "high", "med": "med", "high": "low", "na": "na"}
...
def fetch_district_data_cached(month_year: str) -> pd.DataFrame:
    return fetch_district_data(month_year)
...
def fetch_district_data(month_year: str) -> pd.DataFrame:
    ...
    df["CMS_MARKET_SIZE"] = pd.to_numeric(df["CMS_MARKET_SIZE"], errors="coerce").fillna(0)
    ...
    SELECT
      final_district AS District,
      round(COALESCE(SUM(cms_gtv_success) / 1e7, 0),3) AS CMS_GTV
    FROM `spicemoney-dwh.analytics_dwh.csp_monthly_timeline_with_tu` t1
    LEFT JOIN `spicemoney-dwh.analytics_dwh.v_client_pincode` t2
      ON t1.agent_id = t2.retailer_id
    WHERE t1.month_year = @month_year
    ...
    q_rwco = """
    WITH reverse_wallet_data AS (
    SELECT
        District,
        COALESCE(SUM(amount), 0) AS RW_AMT
    FROM (
        SELECT c.*, d.final_district AS District
        FROM (
        SELECT a.*, b.md_code AS distributor_id
        FROM (
            SELECT t1.*, t2.client_id
            FROM (
            SELECT
                DATE(transfer_date) AS date_,
                retailer_wallet_id,
                distributor_retailer_trans_id AS trans_id,
                amount_transferred AS amount
            FROM `spicemoney-dwh.prod_dwh.cme_wallet_distributor_fund`
            WHERE DATE_TRUNC(DATE(transfer_date), MONTH) = @month_year
            ) t1
            LEFT JOIN `spicemoney-dwh.prod_dwh.client_wallet` t2
            ON t1.retailer_wallet_id = t2.client_wallet_id
        ) a
        LEFT JOIN (
            SELECT retailer_id, md_code
            FROM `spicemoney-dwh.prod_dwh.client_details`
            WHERE client_type = 'retailer'
        ) b
            ON a.client_id = b.retailer_id
        ) c
        LEFT JOIN `spicemoney-dwh.analytics_dwh.v_client_pincode` d
        ON c.client_id = d.retailer_id
    )
    GROUP BY District
    ),
    ...
    SELECT
        District,
        COALESCE(SUM(CO_GTV), 0) AS CO_GTV
    FROM (
        SELECT c.*, d.final_district AS District
        FROM (
        SELECT
            agent_id,
            month_year,
            (aeps_gtv_success + ap_gtv_success + matm_gtv_success) AS CO_GTV
        FROM `spicemoney-dwh.analytics_dwh.csp_monthly_timeline_with_tu`
    ...
    SELECT
    COALESCE(t1.District, t2.District) AS District,
    ROUND(SAFE_DIVIDE(RW_AMT, CO_GTV) * 100, 1) AS RW_CO_RATIO
    FROM reverse_wallet_data t1
    JOIN co_amt t2
    ON t1.District = t2.District
    """

    df_rwco = client.query(q_rwco, job_config=job_config).result().to_dataframe()
    df_rwco["District"] = df_rwco["District"].astype(str).str.upper().str.strip()

    df = df.merge(df_rwco, left_on="DISTRICT_NAME", right_on="District", how="left")
    df.drop(columns=["District"], inplace=True, errors="ignore")
    df["RW_CO_RATIO"] = df["RW_CO_RATIO"].fillna(0)
    ...
    focus_month_yyyymm = pd.to_datetime(month_year).strftime("%Y%m")

    job_config_sp_churn = bigquery.QueryJobConfig(
    query_parameters=[
        bigquery.ScalarQueryParameter("month_year", "DATE", month_year),
        bigquery.ScalarQueryParameter("month_yyyymm", "STRING", focus_month_yyyymm),
        ]
        )
    ...
    sp_churn_data AS (
    SELECT agent_id
    from `spicemoney-dwh.analytics_dwh.expected_gtv_churn_simulation_model_v`
    --FROM `spicemoney-dwh.ds_bi_dwh.Expected_gtv_churn_simulation_model`
    WHERE month_year = @month_yyyymm
        AND churn_flag = 1
        AND agent_id IN (SELECT agent_id FROM sp_data)
    )
    ...
    CHURNED_SMAs,
    ALL_SPs,
    ROUND(SAFE_DIVIDE(CHURNED_SMAs, ALL_SPs) * 100, 2) AS SP_USAGE_CHURN_PERC
    FROM (
    SELECT
        District,
        COUNT(DISTINCT CASE WHEN churn_tag = 'CHURN' THEN agent_id END) AS CHURNED_SMAs,
        COUNT(DISTINCT agent_id) AS ALL_SPs
    FROM (
        SELECT c.*, d.final_district AS District
        FROM (
        SELECT
            a.agent_id,
            CASE WHEN b.agent_id IS NOT NULL THEN 'CHURN' ELSE 'NOT CHURN' END AS churn_tag
        FROM sp_data a
        LEFT JOIN sp_churn_data b
    ...
    df_spch = client.query(q_sp_usage_churn, job_config=job_config_sp_churn).result().to_dataframe()
    df_spch["District"] = df_spch["District"].astype(str).str.upper().str.strip()

    df = df.merge(df_spch[["District", "SP_USAGE_CHURN_PERC"]], left_on="DISTRICT_NAME", right_on="District", how="left")
    df.drop(columns=["District"], inplace=True, errors="ignore")
    df["SP_USAGE_CHURN_PERC"] = df["SP_USAGE_CHURN_PERC"].fillna(0)
...
def generate_geo_spatial_map(
    ...
    # df = fetch_district_data_cached(month_year)
    needed_cols = [METRICS[reference_metric_ui], METRICS[achievement_metric_ui]]
    df = fetch_district_data_cached(month_year)
    ...
    # If achievement KPI is "lower is better", flip its bin before applying the color matrix
    if achievement_metric_ui in REVERSE_ACHIEVEMENT_METRICS:
        merged["ach_bin"] = merged["ach_bin"].map(lambda b: ACH_BIN_FLIP.get(b, b))
    ...
    "CMS GTV": {"high": "> 5 Cr", "med": "1 – 5 Cr", "low": "< 1 Cr"},
    "Monthly Visit Coverage" : {
    "high": "> 10%",
    "med": "4 – 10%",
    "low": "< 4%",
...
def main():
    ...
    with st.sidebar:
        ...
        states = ["ANDHRA PRADESH", "KERALA", 
            ...
            "MAHARASHTRA", "KARNATAKA", "ODISHA", "CHATTISGARH",
